...
def run_prediction(custom_prompt: str = None):
    ...
    market_report = runner.run(market_agent, "現在の日経平均(^N225)と関連指数、ドル円のデータを報告して")
    
    print("Analyzing News...")
    news_report = runner.run(news_agent, "日経平均に関連する最新ニュースとセンチメントを報告して")
    
    print("Technical Analysis...")
    tech_report = runner.run(tech_agent, "日経平均(^N225)のテクニカル指標とシグナルを分析して")
    
    print("Economic Sentiment...")
    econ_report = runner.run(econ_agent, "現在の世界経済状況と日経平均への影響をマクロ視点で分析して")
...
"""
    ...
    final_prediction = runner.run(chief_agent, final_input)
    
    print("\n==========================================")
    print("FINAL PREDICTION")
    print("==========================================")
    print(final_prediction.text)
    print("==========================================\n")
